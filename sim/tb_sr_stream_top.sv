...
module tb_sr_stream_top;
    ...
    initial begin
        ...
        if (out_count != OUT_BEATS)
            $fatal(1, "Output beat count mismatch: got %0d expected %0d", out_count, OUT_BEATS);
        if (user_count != 1)
            $fatal(1, "SOF count mismatch: got %0d expected 1", user_count);
        if (last_count != OUT_LINES)
            $fatal(1, "EOL count mismatch: got %0d expected %0d", last_count, OUT_LINES);
        $display("PASS sr_stream_top: out=%0d sof=%0d eol=%0d", out_count, user_count, last_count);
        $finish;
    end
endmodule
